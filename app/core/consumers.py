...
from app.core.utils import vote_event, greeting_event, error_event, start_event, define_event, group_by, winner_event, \
    answer_accepted_event, connection_event, score_event, pause_event, resume_event, validate_event


# TODO change player on player id, room on room id
# TODO try async consumer
# TODO logging into file
# TODO add delay messages
...
class RoomConsumer(JsonWebsocketConsumer):
    ...
    def send_delayed_message(self, event, delay):
        ...
        task = await send_delayed_message.apply_async(args=[self.room_group_name, event], countdown=delay)
        return task
    ...
    def start(self):
        ...
        for i in range(player_count):
            PlayerTask.objects.create(task_id=game_tasks[i], player_id=players[i], scope_cost=scope_cost)
            PlayerTask.objects.create(task_id=repetitive_tasks[i], player_id=players[i], scope_cost=scope_cost)
    ...
    def answer(self, data):
        for answer in data['answer']:
            PlayerTask.objects.get(player=self.player, task_id=answer['questionId']).set_answer(answer['answer'])
        async_to_sync(self.channel_layer.group_send)(self.room_group_name, {
            'type': 'send_message',
    ...
    def vote(self, data):
        for vote in data['votes']:
            PlayerTask.objects.get(task_id=vote['questionId'], player_id=vote['voteId']).likes.add(self.player)
        if PlayerTask.objects.is_vote_ends(self.room):  # check timeout
            for task in Task.objects.filter(playertasks__player__room=self.room,
